...
doc("usage_op_chg_level", "Charge Level", "Assigned units will be commanded to bring the energy storage system up to the requested Charge Target.")
doc("usage_op_gen_rate", "Generation Rate", "Assigned units will be commanded to maintain the requested Generation Target.")
note("The rate used is the input rate into the energy storage system, so using other power generation sources may disrupt this control mode.")
sect("Start and Stop")
...
doc("usage_waste_sps_lc", "Low Charge SPS", "This option prevents the facility waste control from stopping antimatter production at low energy storage system charge (< 10%, resumes after reaching 15%).")
...
doc("fault", "PLC Hardware Fault", "Indicates if the RPS tripped due to a peripheral access fault. Something went wrong interfacing with the reactor, try restarting the PLC.")
doc("sys_fail", "Reactor System Fault", "Indicates if the RPS tripped due to the reactor not being formed. Ensure that the multi-block is formed.")
...
doc("high_temp", "Core Temperature High", "Indicates if the RPS tripped due to reaching damaging temperatures. Await damage levels to lower.")
...
doc("ess_computed_status", "Energy Storage System", "The energy storage system (induction matrix or energy core) is connected.")
...
doc("as_ess_fault", "ESS Hardware Fault", "Automatic SCRAM occurred due to the loss of the energy storage connection, or it being unformed or faulted.")
doc("as_ess_fill", "ESS Charge High", "Automatic SCRAM occurred due to energy storage charge exceeding acceptable limit.")
...
doc("ui_fac_auto_cr", "Charge Range", "When set to Charge Range mode, assigned units will run once the energy storage system charge percentage falls to the start threshold until it reaches the stop threshold, keeping it within the range.")
doc("ui_fac_auto_ct", "Charge Target", "When set to Charge Level mode, assigned units will run to reach and maintain this energy storage system charge level.")
doc("ui_fac_auto_gt", "Gen. Target", "When set to Generation Rate mode, assigned units will run to reach and maintain this continuous power output, per the sum of assigned unit turbine generation rate.")
...
doc("ui_fac_auto_cl", "Charge Level", "This runs assigned units to maintain an energy storage system charge level.")
doc("ui_fac_auto_gr", "Generation Rate", "This runs assigned units to meet a target combined assigned unit turbine generation rate.")
...
doc("ui_fac_waste_sps_lc_act", "SPS Disabled LC", "When the system is falling back to polonium production to prevent draining all power with the SPS while the energy storage system charge has dropped below 10% and not yet reached 15%.")
doc("ui_fac_waste_pu_fall", "Pu Fallback", "Switch from Po or Antimatter when the SNAs can't keep up (like at night).")
doc("ui_fac_waste_sps_lc", "Low Charge SPS", "Continue running antimatter production even at low energy storage system charge levels (<10%).")
...
doc("ui_fac_im_discharge", "Discharging", "Charge is draining (more output than input).")
doc("ui_fac_im_maxio", "Max I/O Rate", "The induction providers are at their maximum rate.")
doc("ui_fac_im_eta", "ETA", "The ETA is based off a longer average so it may take a minute to stabilize, but will give a rough estimate of time to charge/discharge.")
sect("Energy Core")
text("The energy core statistics are shown at the bottom right, including a fill bar for the FILL.")
text("Averages are computed by the system while other data is directly from the device.")
doc("ui_fac_ec_charge", "Charging", "Charge is increasing (more input than output).")
doc("ui_fac_ec_discharge", "Discharging", "Charge is draining (more output than input).")
doc("ui_fac_ec_eta", "ETA", "The ETA is based off a longer average so it may take a minute to stabilize, but will give a rough estimate of time to charge/discharge.")
...
doc("G_ESS", "ESS", "Energy Storage System.")
doc("G_FP", "FP", "Front Panel.")
doc("G_Hi", "Hi", "High.")
doc("G_Lo", "Lo", "Low.")
...
doc("G_EnergyStorage", "Energy Storage System", "The system used for storing energy, such as an induction matrix or an energy core.")
