...
function update.pre_auto()
    ...
    if self.induction[1] or self.ecore[1] then
        ...
        if faulted then
            -- a fault occurred, cannot reliably update stats
            has_data = false
            self.ess_stat_init = false
            self.ess_faulted_times = { build_update, rate_update, charge_update }
        elseif not self.ess_stat_init then
            -- prevent operation with partially invalid data
            -- all fields must have updated since the last fault
            has_data = self.ess_faulted_times[1] < build_update and
                       self.ess_faulted_times[2] < rate_update and
                       self.ess_faulted_times[3] < charge_update
        end

        if has_data then
            self.ess_percent = percent

            if self.ess_stat_init then
...
function update.auto_safety()
    ...
    if self.induction[1] or self.ecore[1] then
        ...
        local was_fill = astatus.ess_fill
    ...
    if (self.mode ~= PROCESS.INACTIVE) and (self.mode ~= PROCESS.SYSTEM_ALARM_IDLE) then
        local scram = astatus.ess_fault or astatus.ess_fill or astatus.crit_alarm or astatus.radiation or astatus.gen_fault
